...
app.use(
  session({
    secret: process.env.SESSION_SECRET || "babcock_publishing_secret_key",
    resave: false,
...
// JWT Secret
const JWT_SECRET = process.env.JWT_SECRET || "babcock_publishing_jwt_secret";
...
const initDatabase = async () => {
  ...
  try {
    ...
    const [adminRows] = await pool
      .promise()
      .query("SELECT id FROM users WHERE email = ?", ["admin@babcock.edu.ng"]);
...
app.post("/api/users/register", async (req, res) => {
  try {
    ...
    const [result] = await pool.promise().query(
      `INSERT INTO users (username, email, password, full_name, role, status, verification_token) 
       VALUES (?, ?, ?, ?, 'user', 'pending', ?)`,
      [username, email, hashedPassword, full_name, verificationToken],
    );
...
app.post("/api/users/login", async (req, res) => {
  try {
    ...
    let profile = {};
    if (user.role === "author") {
      const [authorProfile] = await pool
        .promise()
        .query("SELECT * FROM authors WHERE user_id = ?", [user.id]);
...
// Admin Login (updated for new users table)
...
app.post("/api/authors/register", async (req, res) => {
  try {
    ...
    // Check if staff_id already exists in authors
    if (staff_id) {
      const [existingStaffId] = await pool
        .promise()
        .query("SELECT id FROM authors WHERE staff_id = ?", [staff_id]);
    ...
    const [userResult] = await pool.promise().query(
      ...
      [email, email, hashedPassword, full_name],
    );

    // Create author profile
    const [authorResult] = await pool.promise().query(
      `INSERT INTO authors 
       (user_id, staff_id, faculty, department) 
       VALUES (?, ?, ?, ?)`,
      [userResult.insertId, staff_id, faculty, department],
    );
...
app.get("/api/admin/dashboard/stats", authMiddleware, async (req, res) => {
  try {
    ...
    const [
      ...
      pool.promise().query("SELECT COUNT(*) as count FROM books"),
      pool
        .promise()
        .query(
    ...
    const [recentSubmissions] = await pool
      .promise()
      .query(
...
app.get("/api/admin/authors", authMiddleware, async (req, res) => {
  try {
    ...
    if (search) {
      query += " AND (full_name LIKE ? OR email LIKE ? OR staff_id LIKE ?)";
      countQuery +=
        " AND (full_name LIKE ? OR email LIKE ? OR staff_id LIKE ?)";
      const searchTerm = `%${search}%`;
      params.push(searchTerm, searchTerm, searchTerm);
      countParams.push(searchTerm, searchTerm, searchTerm);
    }
    ...
    const [authors] = await pool.promise().query(query, params);
    const [[{ total }]] = await pool.promise().query(countQuery, countParams);
    ...
    for (const author of authors) {
      const [[{ bookCount }]] = await pool
        .promise()
        .query("SELECT COUNT(*) as bookCount FROM books WHERE author_id = ?", [
          author.id,
        ]);
...
app.put("/api/admin/authors/:id/status", authMiddleware, async (req, res) => {
  try {
    ...
    const [result] = await pool
      .promise()
      .query("UPDATE authors SET status = ? WHERE id = ?", [
        status,
        req.params.id,
      ]);
...
app.get("/api/admin/books", authMiddleware, async (req, res) => {
  try {
    ...
    const [books] = await pool.promise().query(query, params);
    const [[{ total }]] = await pool.promise().query(countQuery, countParams);
    ...
    for (const book of books) {
      ...
      // Get sales count
      const [[{ salesCount }]] = await pool
        .promise()
        .query("SELECT COUNT(*) as salesCount FROM sales WHERE book_id = ?", [
          book.id,
        ]);
...
app.get("/api/admin/books/:id", authMiddleware, async (req, res) => {
  try {
    ...
    const [inventory] = await pool
      .promise()
      .query("SELECT * FROM inventory WHERE book_id = ?", [book.id]);
...
app.get("/api/admin/submissions", authMiddleware, async (req, res) => {
  try {
    ...
    const [submissions] = await pool.promise().query(query, params);
    const [[{ total }]] = await pool.promise().query(countQuery, countParams);
...
app.get("/api/admin/contracts", authMiddleware, async (req, res) => {
  try {
    ...
    const [contracts] = await pool.promise().query(query, params);
...
app.get("/api/admin/training", authMiddleware, async (req, res) => {
  try {
    ...
    const [registrations] = await pool.promise().query(query, params);
...
app.get("/api/admin/sales", authMiddleware, async (req, res) => {
  try {
    ...
    const [sales] = await pool.promise().query(query, params);
...
app.get("/api/admin/inventory", authMiddleware, async (req, res) => {
  try {
    ...
    const [inventory] = await pool.promise().query(query, params);
...
app.get("/api/admin/reports/financial", authMiddleware, async (req, res) => {
  try {
    ...
    // Royalties due
    const [royalties] = await pool.promise().query(`
      SELECT 
        r.*, c.contract_number,
...
app.get("/api/admin/settings", authMiddleware, async (req, res) => {
  try {
    const [settings] = await pool
      .promise()
      .query("SELECT * FROM settings ORDER BY category, setting_key");
...
app.get("/api/admin/search", authMiddleware, async (req, res) => {
  try {
    ...
    // Search contracts
    const [contracts] = await pool.promise().query(
      `
...
app.get("/api/author/dashboard", authMiddleware, async (req, res) => {
  try {
    ...
    // Get author ID
    const [authorRows] = await pool
      .promise()
      .query("SELECT id FROM authors WHERE user_id = ?", [userId]);
...
app.get("/api/training", authMiddleware, async (req, res) => {
  try {
    ...
    const [trainings] = await pool.promise().query(query, params);
    const [[{ total }]] = await pool.promise().query(countQuery, countParams);
...
app.get("/api/production", authMiddleware, async (req, res) => {
  try {
    ...
    const [tasks] = await pool.promise().query(query, params);
    const [[{ total }]] = await pool.promise().query(countQuery, countParams);
...
app.get("/api/royalties", authMiddleware, async (req, res) => {
  try {
    ...
    const [royalties] = await pool.promise().query(query, params);
    const [[{ total }]] = await pool.promise().query(countQuery, countParams);
...
app.get("/api/admin/authors", authMiddleware, async (req, res) => {
  try {
    ...
    // Build query
    let query = `
      SELECT a.*, u.full_name, u.email, u.username, u.status as user_status,
    ...
    let countQuery =
      "SELECT COUNT(*) as total FROM authors a JOIN users u ON a.user_id = u.id WHERE 1=1";
    let filteredCountQuery =
      "SELECT COUNT(*) as filtered FROM authors a JOIN users u ON a.user_id = u.id WHERE 1=1";
    ...
    if (status) {
      ...
      filteredCountQuery += " AND u.status = ?";
      params.push(status);
    ...
    if (faculty) {
      ...
      filteredCountQuery += " AND a.faculty = ?";
      params.push(faculty);
      filteredParams.push(faculty);
    }
    ...
    if (search) {
      ...
      const searchTerm = `%${search}%`;
      params.push(searchTerm, searchTerm, searchTerm);
      filteredParams.push(searchTerm, searchTerm, searchTerm);
    }
    ...
    if (order.length > 0 && columns.length > 0) {
      ...
      if (columnMap[orderColumnIndex]) {
        query += ` ORDER BY ${columnMap[orderColumnIndex]} ${orderDirection}`;
      }
    } else {
    ...
    const [[{ filtered }]] = await pool
      .promise()
      .query(filteredCountQuery, filteredParams);
...
📊 DATABASE STATUS:
   ...
   - Tables: 12+ publishing management tables
